...
// Returns true only when the users table is completely empty — meaning this
// is a genuinely fresh database that has never been seeded before.
// This prevents seedDatabase()'s destructive DELETE/reset statements from
// ever running against a live database that already has real user data
// (e.g. on every restart in production, where migrations must still run
// every boot, but seeding must only ever run once).
...
export async function seedDatabase() {
  ...
  // (Must run BEFORE the student_activities seed below, since those rows
  // reference this club via target_club_id — a foreign key requires
  // the club row to already exist.)
  await query(
    `
